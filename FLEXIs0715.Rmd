...
```
```{r}
# number of FLEXIs has RBP site or AGO_CCR site or DICER_CCR site in 4cells+plasma
dim(dat[rowMaxs(as.matrix(dat[,88:92]))>0 & (dat$RBP!="." | dat$AGO_CCR!="." | dat$DICER_CCR!="."),])
#number of differebt RBPs detecetd (including AGO1-4(treated as one) and DICER),so the number should be
# the out put of the following command -1 (the ".") +2 (AGO1-4 and DICER)
length(lapply(strsplit(paste(dat$RBP,collapse = ","), ","), unique)[[1]])-1+2
#Number of AGO_CCR (FLEXIs), DICER_CCR, and agotrpn/mirtron in them
dim(dat[dat$AGO_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0,])
dim(dat[dat$AGO_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0 & dat$Is_agotron!=".",])
dim(dat[dat$DICER_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0,])
dim(dat[dat$DICER_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0 & dat$Is_mirtron!=".",])
dim(dat[dat$DICER_CCR!="." & dat$AGO_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0,])
dim(dat[dat$DICER_CCR!="." & dat$AGO_CCR!="." & rowMaxs(as.matrix(dat[,88:92]))>0 &
          dat$Is_mirtron!="." & dat$Is_agotron!=".",])
#number of FLEXIs with high/low (0.5) PhastCons30 score
dim(dat[rowMaxs(as.matrix(dat[,88:92]))>0 & dat$PhastCon30>=0.5,])
dim(dat[rowMaxs(as.matrix(dat[,88:92]))>0 & dat$PhastCon30<0.5,])
dim(dat[rowMaxs(as.matrix(dat[,88:92]))>0 & dat$PhastCon30>=0.99,])
```
...
dat[,4:6]<-t(t(dat[,4:6])/Unfrag_total)
FLEXI_CPM<-merge(dat[,c(1,4:6)],FLEXI_by_GID,by=1,all.y=T)
FLEXI_CPM[is.na(FLEXI_CPM)]<-0
colnames(FLEXI_CPM)[2:4]<-paste0(colnames(dat[,4:6]),"_UF")
colnames(FLEXI_CPM)[5:7]<-paste0(colnames(dat[,4:6]),"_FLEXI")


postscript("Figures/4cells_ver2.eps",width = 15,height=10,horizontal = F)
...
text(-5,12,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))
...
FLEXI_by_GID<-FLEXI_CPM[!(FLEXI_CPM[,5]==2^-10 & FLEXI_CPM[,6]==2^-10),]
plot(log2(FLEXI_by_GID[,c(5,6)]),xlim=c(-10,5),ylim=c(-10,5))
abline(0,1,col="red")
cor_s=formatC(cor(FLEXI_by_GID[,5],FLEXI_by_GID[,6],method = "spearman"),digits=2, format="f")
cor_p=formatC(cor(FLEXI_by_GID[,5],FLEXI_by_GID[,6],method = "pearson"),digits=2, format="f")
text(-5,3,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))

FLEXI_by_GID<-FLEXI_CPM[!(FLEXI_CPM[,5]==2^-10 & FLEXI_CPM[,7]==2^-10),]
plot(log2(FLEXI_by_GID[,c(5,7)]),xlim=c(-10,5),ylim=c(-10,5))
abline(0,1,col="red")
cor_s=formatC(cor(FLEXI_by_GID[,5],FLEXI_by_GID[,7],method = "spearman"),digits=2, format="f")
cor_p=formatC(cor(FLEXI_by_GID[,5],FLEXI_by_GID[,7],method = "pearson"),digits=2, format="f")
text(-5,3,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))

FLEXI_by_GID<-FLEXI_CPM[!(FLEXI_CPM[,6]==2^-10 & FLEXI_CPM[,7]==2^-10),]
plot(log2(FLEXI_by_GID[,c(6,7)]),xlim=c(-10,5),ylim=c(-10,5))
abline(0,1,col="red")
cor_s=formatC(cor(FLEXI_by_GID[,6],FLEXI_by_GID[,7],method = "spearman"),digits=2, format="f")
cor_p=formatC(cor(FLEXI_by_GID[,6],FLEXI_by_GID[,7],method = "pearson"),digits=2, format="f")
text(-5,3,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))
dev.off()
```
```{r}
...
```{r}
#by RPM≥0.01
set_1 <- as.character(dat$ID[dat$UHRR/mapped_reads[7]>=0.01])
set_2 <- as.character(dat$ID[dat$K562/mapped_reads[8]>=0.01])
set_3 <- as.character(dat$ID[dat$HEK/mapped_reads[9]>=0.01])
set_4 <- as.character(dat$ID[dat$Hela/mapped_reads[10]>=0.01])
set_5 <- as.character(dat$ID[dat$Plasma/mapped_reads[11]>=0.01])
set <- list ("UHRR"=set_1,
             "K-562"=set_2,"HEK 293T"=set_3,
             "Hela S3"=set_4,"Plasma"=set_5)
m = make_comb_mat(set)

postscript(paste0("Figures/4cell_plasma_Upset_RPM001.eps"),height=4,width=8)
UpSet(m,set_order=c("UHRR","K-562","HEK 293T","Hela S3","Plasma"),
      comb_col = c("tomato","royalblue1","goldenrod","orchid","black")[comb_degree(m)])
dev.off()
set_size(m)
...
1#By gene (RPM≥0.01)
set_1<-unique(sapply(strsplit(set_1,"_"),"[[",2))
set_2<-unique(sapply(strsplit(set_2,"_"),"[[",2))
set_3<-unique(sapply(strsplit(set_3,"_"),"[[",2))
set_4<-unique(sapply(strsplit(set_4,"_"),"[[",2))
set_5<-unique(sapply(strsplit(set_5,"_"),"[[",2))
set <- list ("UHRR"=set_1,
             "K-562"=set_2,"HEK 293T"=set_3,
             "Hela S3"=set_4,"Plasma"=set_5)
m = make_comb_mat(set)

postscript(paste0("Figures/4cell_plasma_Upset_RPM001_by_gene.eps"),height=4,width=8)
UpSet(m,set_order=c("UHRR","K-562","HEK 293T","Hela S3","Plasma"),
      comb_col = c("tomato","royalblue1","goldenrod","orchid","black")[comb_degree(m)])
dev.off()
set_size(m)
...
```
```{r}
# Density of length/GC/MFE/PhastCons, reads>=1

pdf("Figures/Density_plots.pdf",width=12,height=12)
#length
pdf(NULL)
...
#Density plot based on if FLEXI contain RBP
pdf("Figures/Density_by_RBP.pdf",width=12,height=12)
par(mfrow=c(2,2))
#length
FourCellPlasma<-read.delim("4cell_plasma_FLEXI.tsv")
W_RBP<-FourCellPlasma[FourCellPlasma$RBP!="." | FourCellPlasma$AGO_CCR!="." | FourCellPlasma$DICER_CCR!=".",]
WO_RBP<-FourCellPlasma[FourCellPlasma$RB=="." & FourCellPlasma$AGO_CCR=="." & FourCellPlasma$DICER_CCR==".",]
plot(density(W_RBP$Len),bty="n",xlim=c(0,350),lwd=1.5,
     ylim=c(0,0.015),main=NA,xlab="Intron length (bp)",col="red")
lines(density(WO_RBP$Len),xlim=c(0,350),lwd=1.5,col="black")
legend(150,0.012,lty=c(1,1),lwd=1.5,col=c("black","red"),
       legend = c("FLEXIs without RBP sits","FLEXIs with RBP sits"),bty="n")

#GC
plot(density(W_RBP$GC),bty="n",xlim=c(0,100),lwd=1.5,
     ylim=c(0,0.05),main=NA,xlab="Intron length (bp)",col="red")
lines(density(WO_RBP$GC),xlim=c(0,100),lwd=1.5,col="black")

#MEF
plot(density(W_RBP$MFE),bty="n",xlim=c(-150,0),lwd=1.5,
     ylim=c(0,0.03),main=NA,xlab="Minimal free energy (MFE; kcal/mol)",col="red")
lines(density(WO_RBP$MFE),xlim=c(-150,0),lwd=1.5,col="black")

#PhastCons
plot(density(W_RBP$PhastCon30),bty="n",xlim=c(0,1),lwd=1.5,
     ylim=c(0,7),main=NA,xlab="PhastCons score",col="red")
lines(density(WO_RBP$PhastCon30),lwd=1.5,col="black")
dev.off()

#Density plots of high/low phastCons, 0.8
...
dev.off()
#RBP top20 in allFLEX ang whole genome
pdf("Figures/RBP_top20_by_cell.pdf",height=8,width=8)
par(mfrow=c(2,2))
Fun<-RBP[,c(1,46:49)]
...
pdf("Figures/RBP_top20_by_genome.pdf",height=8,width=8)
par(mfrow=c(2,2))
Fun<-RBP[,c(1,46:49)]
Fun<-cbind(Fun$RBP.name,data.frame(100*prop.table(as.matrix(Fun[,2:5]),margin = 2)))
...
for (i in 1:length(RBP_list)){
  ...
  } else if (RBP_list[i] =="DICER") {
    temp1<-FourCellPlasma[FourCellPlasma$DICER_CCR!=".",]
    temp2<-FourCellPlasma[FourCellPlasma$DICER_CCR==".",]
  } else {
    temp1<-FourCellPlasma[grep(RBP_list[i],FourCellPlasma$RBP),]
    temp2<-FourCellPlasma[grep(RBP_list[i],FourCellPlasma$RBP,invert=T),]
  }
  Len_t<-0
  GC_t<-0
  MFE_t<-0
  PhastCon30_t<-0
  for (inter in 1:100){
    sample_size<-Fun$RBP_by_FLEXI[Fun$Name==RBP_list[i]]
    test_set<-FourCellPlasma[sample(1:8144,sample_size),]
    if (wilcox.test(temp1$Len,test_set$Len)$p.value>0.05) {Len_t=Len_t+1}
    if (wilcox.test(temp1$GC,test_set$GC)$p.value>0.05) {GC_t=GC_t+1}
    if (wilcox.test(temp1$MFE,test_set$MFE)$p.value>0.05) {MFE_t=MFE_t+1}
    if (wilcox.test(temp1$PhastCon30,test_set$PhastCon30)$p.value>0.05) {PhastCon30_t=PhastCon30_t+1}
  }
  #Length
  ...
  legend(120,0.012,lty=c(1,1),lwd=1.5,col=c("red","black"),
         legend = c(paste0(RBP_list[i]," (",Fun$RBP_by_FLEXI[Fun$Name==RBP_list[i]],")"),"Others"),bty="n")
  if (Len_t==0) {
...
dev.off()
# added plots for FDR<=0.05
pdf("RBP_density_sig.pdf",width=11,height=8,onefile = T)
RBP_list_sig<-RBP_list[c(41,48,52,9,13,18,25,28,29,36,43,44,1:8,10:12,14:17,
                         19:24,26:27,30:35,37:40,42,45:47,49:51,53)]
par(mfrow=c(3,4))
...
dev.off()
#add additional panel for RBP clustering by host genes
...
RBP_53[6,27]<-1
pdf("Figures/RBP_by_category.pdf",width=8,height=8)
...
axis(1,las=2,at = 1:53,labels = RBP_53$RBP.name,tick = FALSE)
dev.off()
```
```{r}
dat<-read.delim("all.FLEXI")
dat<-dat[,c(8,82:87)]
dat<-dat[rowSums(dat[,2:7])>0,]
...
dat$HEK<-rowSums(gene_counts[,c(42:49)])
dat$Hela<-rowSums(gene_counts[,c(50:59)])
Unfrag_total<-colSums(dat[,4:9])/1e6
rownames(dat)<-dat$ID
dat$ID<-as.character(dat$ID)
Unfrag_by_FLEXI<-merge(dat[,c(1,4:9)],FLEXI_by_GID[,1],by=1,all.y=T)
...
FLEXI_by_GID[,2:7]<-t(t(FLEXI_by_GID[,2:7])/Unfrag_total)
Unfrag_by_FLEXI[,2:7]<-t(t(Unfrag_by_FLEXI[,2:7])/Unfrag_total)
Frag_by_FLEXI[,2:5]<-t(t(Frag_by_FLEXI[,2:5])/Frag_total)
colnames(Frag_by_FLEXI)<-colnames(Unfrag_by_FLEXI[1:5])
