...
knitr::opts_chunk$set(echo = TRUE,self_contained=F)
library(gplots)
...
set <- list ("UHRR"=set_1,"K-562"=set_2,"HEK 293T"=set_3,"Hela S3"=set_4,"Plasma"=set_5)
vennplot1 <- venn.diagram (set, filename=NULL,category.names=names(set),
                           cat.col = col[1:5], main=paste0("FLEXI RNAs (>= ",CPM_cut_off," reads)"),
                           fill = col[1:5],
                           height = 300, width = 300, units = "px",
                           cex = 1,cat.pos=c(0,0,180,180,0),
                           main.cex=1, cat.cex = 1) 
...
set <- list ("UHRR"=set_1,"K-562"=set_2,"HEK 293T"=set_3,"Hela S3"=set_4,"Plasma"=set_5)
vennplot3 <- venn.diagram (set, filename=NULL,category.names=names(set),
                           cat.col = col[1:5], main=paste0("FLEXI RNAs (>= ",CPM_cut_off," reads) containing genes"),
                           fill = col[1:5],
                           height = 300, width = 300, units = "px",
                           cex = 1,cat.pos=c(0,0,180,180,0),
                           main.cex=1, cat.cex = 1) 
...
```{r}
#RBP
RBP<-read.delim("../JA20150_combined/150_RBP_AGO_DICER.info")
Cell<-read.table("../JA20150_combined/4cell_plasma_RBP.counts",col.names=c("ID","Cells"))
FLEXI<-read.table("../JA20150_combined/all_FLEXI_RBP.counts",col.names=c("ID","All_FLEXI"))
Intron<-read.table("../JA20150_combined/all_intron_RBP.counts",col.names=c("ID","All_Intron"))
Genome<-read.table("../JA20150_combined/GRCh38_RBP.counts",col.names=c("ID","GRCh38"))
...
```
```{r}
#hall mark geneset p-value heatmap
Hall<-read.delim("../JA20150_combined/Hallmark/cells_hallmark.txt")
pdf("Figures/hallmark_heatmap.pdf",height=15,width=6)
#heatmap(log10(as.matrix(Hall[,2:7])),labRow = Hall$Hallmark,Colv = NA,keep.dendro=F,
#        col = colorRampPalette(brewer.pal(10, "Spectral")))
#dev.off()
Col<-colorRampPalette(c(rev(brewer.pal(11, "RdYlBu")[3:8])))(50)

#heatmap.2(log2(as.matrix(Hall[,2:7])),labRow = Hall$Hallmark,dendrogram = "none",scale="none",margins = c(10, 20),
#          density.info = "none",trace="none",symm=F,symbreaks=F,keysize=1,Colv = F,symkey=F,
#          col = rev(Col))
heatmap.2(as.matrix(Hall[,2:5]),labRow = Hall$Hallmark,dendrogram = "none",scale="none",margins = c(10, 20),
          density.info = "none",trace="none",symm=F,symbreaks=F,keysize=1,Colv = F,symkey=F,
          col = rev(Col))
dev.off()
```
...
gene_counts<-read.delim("../JA20150_combined/combined_run.counts")
dat<-gene_counts[,1:3]
...
Frag_by_FLEXI<-read.delim("../IBC/all_IBC.counts")
Frag_by_FLEXI<-Frag_by_FLEXI[,c(1:3,6:7,10:11)]
rownames(Frag_by_FLEXI)<-Frag_by_FLEXI$ID
...
postscript("Figures/H_Vs_C.eps",width = 15,height=10,horizontal = F)
...
points(log2(dat[dat$PatientA_H==2^-10 & dat$PatientA_C>=0.01 & dat$PatientA_C_repro,c(11,13)]),col="red")
...
text(-8,4,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))
points(log2(dat[dat$PatientB_H==2^-10 & dat$PatientB_C>=0.01 & dat$PatientB_C_repo,c(12,14)]),col="red")
dev.off()
```
```{r}
#Upset plot
