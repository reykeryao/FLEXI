...
FLEXI_CPM$K562_repo<-rowMedians(as.matrix(dat[,56:63]))>0
FLEXI_CPM$HEK_repo<-rowMedians(as.matrix(dat[,64:71]))>0
FLEXI_CPM$Hela_repo<-rowMedians(as.matrix(dat[,72:81]))>0
...
sig.x<-FLEXI_by_GID[FLEXI_by_GID$K562>=0.03 &
                      FLEXI_by_GID$Hela==2^-10 &
                      FLEXI_by_GID$K562_repo,]
sig.y<-FLEXI_by_GID[FLEXI_by_GID$Hela>=0.03 &
                      FLEXI_by_GID$K562==2^-10 &
                      FLEXI_by_GID$Hela_repo,]
...
sig.x<-setdiff(sig.x,sig.y)
sig.y<-setdiff(sig.y,sig.all)
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.y,c(2,4)]),col="red")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(2,4)]),col="blue")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.all,c(2,4)]),col="green")
...
text(1,-7,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))
sig.x<-FLEXI_by_GID[FLEXI_by_GID$HEK>=0.03 &
                      FLEXI_by_GID$Hela==2^-10 &
                      FLEXI_by_GID$HEK_repo,]
sig.y<-FLEXI_by_GID[FLEXI_by_GID$Hela>=0.03 &
                      FLEXI_by_GID$HEK==2^-10 &
                      FLEXI_by_GID$Hela_repo,]
points(log2(sig.y[,c(3,4)]),col="red")
points(log2(sig.x[,c(3,4)]),col="blue")
#Unfrag scatter
#FELXIs between HEK and HeLa
...
sig.x<-setdiff(sig.x,sig.y)
sig.y<-setdiff(sig.y,sig.all)
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.y,c(3,4)]),col="red")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(3,4)]),col="blue")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.all,c(3,4)]),col="green")
...
sig.x<-FLEXI_by_GID[FLEXI_by_GID$K562>=0.03 &
                      FLEXI_by_GID$HEK==2^-10 &
                      FLEXI_by_GID$K562_repo,]
sig.y<-FLEXI_by_GID[FLEXI_by_GID$HEK>=0.03 &
                      FLEXI_by_GID$K562==2^-10 &
                      FLEXI_by_GID$HEK_repo,]
points(log2(sig.y[,c(2,3)]),col="red")
points(log2(sig.x[,c(2,3)]),col="blue")
...
sig.x<-unique(sig.x$GID)
sig.y<-unique(sig.y$GID)
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.y,c(2,3)]),col="red")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(2,3)]),col="blue")
dev.off()
#remove the temporay object created for this plot
rm(list=c("Cell_counts","FLEXI_by_GID","FLEXI_CPM","cor_p","cor_s",
          "Unfrag_total","GID_list","sig.x","sig.y"."sig.all"))
...
pdf("Figures/FigS4.pdf",width=10,height=10)
par(mfrow=c(2,2))
for (i in 1:4){
...
#Density plot,Fig1C right 3 panels
...
dev.off()
#remove objects
rm(list=c("GC.line","length.line","MFE.line","col"))
...
dev.off()
rm(list=c("snoRNA","snRNA","i","U7","U11","SNORD74","SNORD78","RN7SK","RN7SL",
          "YRNA","VTRNA","RMRP","RPPH1","snc","j","D_height"))

#linear regression model of sncRNA/FLEXIs, FigS6
gene_counts<-read.delim("combined_counts.tsv")
...
pdf("Figures/FigS6.pdf",height=8,width=8)
...
dev.off()
rm(list=c("temp","gene_for_correlation","gene_counts","scol","y","x","i","cor_s",
          "cor_p","newx","lm.out","conf_interval"))
...
#Fig7A and FigS11, FLEXI by 0 cutoff and ≥ 0.01 RPM FLEXIs
cut_off=0.01
...
#FigS11, cutoff 0 RPM, by FLEXI
cut_off=0
postscript("Figures/FigS11_1.eps",height=4,width=8)
set_1 <- as.character(FLEXI_CPM$ID[(FLEXI_CPM$BCH3+FLEXI_CPM$BCH4)>cut_off])
set_2 <- as.character(FLEXI_CPM$ID[FLEXI_CPM$BC3>cut_off])
set_3 <- as.character(FLEXI_CPM$ID[FLEXI_CPM$BC4>cut_off])
set_4 <- as.character(FLEXI_CPM$ID[FLEXI_CPM$MDA>cut_off])
...
UpSet(m,set_order=c("MDA-MB-231","MCF7","Patient A (Cancer)",
      ...
      comb_order=c(6,15,16,13,14,26:21,31:30,29:28,1,5:2,12:7,20:17,27))
dev.off()
postscript(paste0("Figures/FigS11_2.eps"),height=4,width=8)
set_1 <- unique(FLEXI_CPM$GID[(FLEXI_CPM$BCH3+FLEXI_CPM$BCH4)>cut_off])
set_2 <- unique(FLEXI_CPM$GID[FLEXI_CPM$BC3>cut_off])
set_3 <- unique(FLEXI_CPM$GID[FLEXI_CPM$BC4>cut_off])
set_4 <- unique(FLEXI_CPM$GID[FLEXI_CPM$MDA>cut_off])
...
for (i in 1:126){
  Phast[i,4]<-fisher.test(as.matrix(rbind(Phast[i,2:3],R_sum),2,2))$p.value
}
...
abline(0,1,col="red")
text(Phast[Phast$pvalue<=0.05,c(2,3)],pos = 3,cex=0.5,
     labels = Phast$ID[Phast$pvalue<=0.05])
...
text(Phast[Phast$pvalue<=0.05,c(2,3)],pos = 3,cex=0.5,
     labels = Phast$ID[Phast$pvalue<=0.05])
dev.off()
rm(list=c("Phast","Phast99","R_sum"))
#Fig4B
pdf("Figures/Fig4B.pdf",height=6,width=12)
par(mfrow=c(1,2),pty="s",pch=16)
RBP_fre<-RBP[,c(1,46,48,49)]
RBP_fre$All_Intron<-RBP$All_Intron-RBP$All_FLEXI
R_sum<-colSums(RBP_fre[,2:4])
RBP_fre$Bpvlue<-1
RBP_fre$Cpvlue<-1
for (i in 1:152){
  RBP_fre[i,5]<-fisher.test(as.matrix(rbind(RBP_fre[i,c(2,3)],R_sum[c(1,2)]),2,2))$p.value
  RBP_fre[i,6]<-fisher.test(as.matrix(rbind(RBP_fre[i,c(2,4)],R_sum[c(1,3)]),2,2))$p.value
}
...
RBP_fre<-RBP_fre[,c(1,3,2,4,5,6,7)]
plot(RBP_fre[RBP_fre$col==0,2:3],xlim=c(0,20),ylim=c(0,20),cex=1.5,bty="n",
     ylab="FLEXIs (% RBP sites)",xlab="Long introns (% RBP sites)")
points(RBP_fre[RBP_fre$col<1 & RBP_fre$col>0,2:3],col="red",cex=1.5)
points(RBP_fre[RBP_fre$col==1,2:3],col="orange",cex=1.5)
points(RBP_fre[RBP_fre$col>1,2:3],col="skyblue",cex=1.5)
text(RBP_fre[(RBP_fre$Bpvlue<=0.05 & (RBP_fre[,2]>=4 | RBP_fre[,3]>=4)),2:3],
     labels = RBP_fre$RBP.name[RBP_fre$Bpvlue<=0.05& (RBP_fre[,2]>=4 | RBP_fre[,3]>=4)])
abline(0,1,col="red")

plot(RBP_fre[RBP_fre$col==0,c(4,3)],xlim=c(0,20),ylim=c(0,20),cex=1.5,bty="n",
     xlab="All RBP binding sites (%)",ylab="FLEXIs (% RBP sites)")
points(RBP_fre[RBP_fre$col<1 & RBP_fre$col>0,c(4,3)],col="red",cex=1.5)
points(RBP_fre[RBP_fre$col==1,c(4,3)],col="orange",cex=1.5)
points(RBP_fre[RBP_fre$col>1,c(4,3)],col="skyblue",cex=1.5)
abline(0,1,col="red")
text(RBP_fre[(RBP_fre$Cpvlue<=0.05 & (RBP_fre[,4]>=4 | RBP_fre[,3]>=4)),2:3],
     labels = RBP_fre$RBP.name[RBP_fre$Cpvlue<=0.05& (RBP_fre[,4]>=4 | RBP_fre[,3]>=4)])
dev.off()
rm(RBP_fre)
#Fig4A and S7
#All RBPs by occurrence in FLEXIs (log scale)
pdf("Figures/FigS7.pdf",height=12,width=12)
...
RBP_53[6,27]<-1
pdf("Figures/Fig6_2.pdf",width=12,height=8)
...
dev.off()
#FigS10
pdf("Figures/FigS10.pdf",width=11,height=8,onefile = T)
...
#ZINB-WaVE
dat3<-dat[,38:81]
dat3<-dat3[rowSums(dat3)>0,]
Fc<-SummarizedExperiment(assays = list(counts = as.matrix(dat3)),
                         colData = data.frame(label=c(rep(5,2),rep(6:9,each=8),rep(10,10))))
zinb<-zinbwave(Fc)
W <- data.frame(reducedDim(zinb))
W$label<-as.factor(Fc$label)

pdf("Figures/FigS5.pdf",height=3,width=9)
...
#plot PCA
plot(pca$x[,1:2],main="PCA",col=col[dat2$label],
     xlab=NA,ylab=NA,pch=16)
legend(1,6,legend = name, col=col,pch=16,bty="n",cex=0.7)
#plot t-SNE
