...
points(log2(sig.x[,c(2,4)]),col="yellow")
#Unfrag scatter
#FELXIs between K562 and HeLa
...
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(2,4)]),col="yellow")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.all,c(2,4)]),col="orange")
...
text(1,-7,bquote(atop(italic(r)== .(cor_p)~phantom(),italic(r[s])== .(cor_s))))
sig.x<-FLEXI_by_GID[FLEXI_by_GID$HEK>2^-10  &
                      FLEXI_by_GID$Hela==2^-10 &
...
points(log2(sig.x[,c(3,4)]),col="yellow")
#Unfrag scatter
#FELXIs between HEK and HeLa
...
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(3,4)]),col="yellow")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.all,c(3,4)]),col="orange")
...
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.x,c(2,3)]),col="yellow")
points(log2(FLEXI_by_GID[FLEXI_by_GID$ID%in%sig.all,c(2,3)]),col="orange")
dev.off()
...
Phast99<-dat$ID[dat$PhastCon30>=0.99 & rowSums(dat[,88:91])>0]
...
#     labels = Phast$ID[Phast$pvalue<=0.05])
dev.off()
rm(list=c("Phast","Phast99","R_sum","FLEXI","Phast99_fre","RBP_4cell_plasma","RBP_fre"))
#Fig4B
FLEXI<-dat[rowMaxs(as.matrix(dat[,88:91]))>0,]
RBP_4cell_plasma<-read.table("4cell_plasma_combined_RBP.info",col.names=c("ID","RBP"))
RBP_4cell_plasma<-RBP_4cell_plasma[RBP_4cell_plasma$ID%in%FLEXI$ID,]
...
pdf("Figures/Fig4B.pdf",height=8,width=8)
...
abline(0,1,col="red")
text(RBP_fre[(RBP_fre$Bpvlue<=0.01 & (RBP_fre[,2]>=4 | RBP_fre[,3]>=4)),c(3,2)],
     labels = RBP_fre$RBP[RBP_fre$Bpvlue<=0.01 & (RBP_fre[,2]>=4 | RBP_fre[,3]>=4)])
#subpanel
...
points(RBP_fre[RBP_fre$col>1,c(3,2)],col="skyblue",cex=1.5)
abline(0,1,col="red")
text(RBP_fre[(RBP_fre$Bpvlue<=0.01 & (RBP_fre[,2]>=2 | RBP_fre[,3]>=2)),c(3,2)],
     labels = RBP_fre$RBP[RBP_fre$Bpvlue<=0.01 & (RBP_fre[,2]>=2 | RBP_fre[,3]>=2)])
...
abline(0,1,col="red")
text(RBP_fre[(RBP_fre$Cpvlue<=0.01 & (RBP_fre[,4]>=4 | RBP_fre[,2]>=4)),c(4,2)],
     labels = RBP_fre$RBP[RBP_fre$Cpvlue<=0.01 & (RBP_fre[,4]>=4 | RBP_fre[,2]>=4)])
...
points(RBP_fre[RBP_fre$col>1,c(4,2)],col="skyblue",cex=1.5)
abline(0,1,col="red")
text(RBP_fre[(RBP_fre$Cpvlue<=0.01 & (RBP_fre[,4]>=2 | RBP_fre[,2]>=2)),c(4,2)],
     labels = RBP_fre$RBP[RBP_fre$Cpvlue<=0.01 & (RBP_fre[,4]>=2 | RBP_fre[,2]>=2)])
